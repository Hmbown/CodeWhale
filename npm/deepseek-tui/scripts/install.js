...
const OPTIONAL_TIMEOUT_MS = 15_000; // fail fast during npm postinstall --optional
const OPTIONAL_STALL_MS = 5_000; // avoid long hangs when install may continue without binaries
...
function httpRequest(rawUrl, opts = {}) {
  const context = opts.context ?? "runtime";
  const totalTimeoutMs = opts.totalTimeoutMs ?? downloadTimeoutMs(context);
  const stallMs = opts.stallMs ?? downloadStallMs(context);
...
// Unsupported platforms, checksum mismatches, and glibc compatibility errors
// must still fail the install with actionable diagnostics.
...
module.exports = {
  ...
  run,
  _internal: {
    // Test hooks for regression coverage; not part of the wrapper runtime API.
    isOptionalInstall,
