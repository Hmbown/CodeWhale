...
#[cfg(test)]
mod parse_config_toml_str_tests {
    use super::{ProviderKind, parse_config_toml_str};

    #[test]
    fn empty_document_parses() {
        parse_config_toml_str("").expect("empty config");
    }

    #[test]
    fn named_provider_parses_off_the_caller_stack() {
        let parsed = parse_config_toml_str("provider = \"openrouter\"\n").expect("named provider");
        assert_eq!(parsed.provider, ProviderKind::Openrouter);
    }
}
