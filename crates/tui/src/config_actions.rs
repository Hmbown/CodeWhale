//! Runtime setting mutation helpers.

use std::path::PathBuf;

use crate::commands::CommandResult;
use crate::config::{
    ApiProvider, COMMON_DEEPSEEK_MODELS, DEFAULT_XIAOMI_MIMO_BASE_URL,
    XIAOMI_MIMO_PAY_AS_YOU_GO_BASE_URL, normalize_model_name_for_provider,
};
...
use crate::tui::app::{App, AppAction, AppMode, ReasoningEffort, SidebarFocus, VimMode};
...
mod tests {
    ...
    use crate::commands::groups::{
        config::config::config_impl::config_command, config::logout::logout_impl::logout,
        config::mode::mode_impl::mode, config::settings::settings_impl::show_settings,
        config::theme::theme_impl::theme, config::trust::trust_impl::trust,
    };
    use crate::config::Config;
    use crate::config_persistence::config_toml_path;
    ...
    #[test]
    fn config_command_defaults_to_native_editor() {
        let mut app = create_test_app();
        app.session.total_tokens = 1234;
        let result = config_command(&mut app, None);
        assert!(result.message.is_none());
        assert!(matches!(result.action, Some(AppAction::OpenConfigView)));
    }

    #[test]
    fn config_command_native_arg_opens_legacy_editor() {
        let mut app = create_test_app();
        let result = config_command(&mut app, Some("native"));
        assert!(result.message.is_none());
        assert!(matches!(result.action, Some(AppAction::OpenConfigView)));
    }
    ...
    #[test]
    fn config_command_model_updates_app_state() {
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_model_auto_enables_auto_thinking() {
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_model_accepts_future_deepseek_model_id() {
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_model_with_save_flag() {
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_approval_mode_valid_values() {
        let mut app = create_test_app();
        // Test auto
    ...
    #[test]
    fn config_command_approval_mode_invalid_value() {
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_without_save_flag() {
        let _lock = lock_test_env();
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_composer_border_updates_live_app() {
        let _lock = lock_test_env();
        let mut app = create_test_app();
    ...
    #[test]
    fn config_command_rejects_invalid_setting() {
        let _lock = lock_test_env();
        let mut app = create_test_app();
        let result = config_command(&mut app, Some("nonexistent value"));
        assert!(result.is_error);
        assert!(
            result
                .message
    ...
    fn config_command_key_without_value_shows_current_setting() {
        let mut app = create_test_app();
        let result = config_command(&mut app, Some("model"));
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
