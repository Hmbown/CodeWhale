...
impl Command for Subagents {
    ...
    fn execute(&self, app: &mut App, _args: Option<&str>) -> CommandResult {
        super::subagents_impl::subagents(app)
    }
}
...
mod tests {
    ...
    fn info_returns_metadata() {
        let cmd = Subagents;
        let info = cmd.info();
        assert_eq!(info.name, "subagents");
        assert!(info.aliases.contains(&"agents"));
    }

    #[test]
    fn execute_opens_subagent_view() {
        let mut app = test_app();
        let result = Subagents.execute(&mut app, None);
