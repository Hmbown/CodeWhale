...
pub(super) fn import_claude_command(app: &mut App, _arg: Option<&str>) -> CommandResult {
    ...
    if std::fs::create_dir_all(&imports_dir).is_ok() {
        wrote_report = std::fs::write(&report_path, import_claude::report_markdown(&plan)).is_ok();
        if !plan.env_safe.is_empty() {
            wrote_bundle =
                std::fs::write(&bundle_path, import_claude::portable_bundle_json(&plan)).is_ok();
        }
    }
