...
pub fn provider(app: &mut App, args: Option<&str>) -> CommandResult {
    ...
    let Some(target) = ApiProvider::parse(name) else {
        return CommandResult::error(format!(
            "Unknown provider '{name}'. Expected: deepseek, nvidia-nim, openai, atlascloud, wanjie-ark, openrouter, novita, fireworks, siliconflow, moonshot, sglang, vllm, or ollama."
        ));
    };
