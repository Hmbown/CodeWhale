...
pub fn apply_document(
    ...
    if persist {
        ...
        let result = commands::shared::config::set_config_value(app, "default_model", default_model_val, true);
        if result.is_error {
            bail!(
                "{}",
                result
    ...
    if app.status_items != new_status_items {
        ...
        if persist {
            let path = commands::shared::config::persist_status_items(&new_status_items)?;
...
fn apply_reasoning_effort(
    ...
    app.update_model_compaction_budget();
    if persist {
        commands::shared::config::persist_root_string_key(
