...
/// - are not a shebang (`#!`) or Markdown heading (`## â€¦`, `### â€¦`).
///
    ...
    loop {
        ...
        if let Some(evt) = maybe_terminal_event {
            ...
            if let Event::Resize(width, height) = evt {
                ...
                // during a maximizeâ†’windowed transition; the next
...
async fn apply_model_picker_choice(
    ...
    if !model_changed && !effort_changed {
        app.status_message = Some(format!(
            "Model unchanged: {model} Â· thinking {}",
                    ...
                    Err(err) => {
                        app.status_message =
                            Some(if app.ui_locale == crate::localization::Locale::ZhHans {
                                format!("æ— æ³•ä¿å­˜ Fleet é…ç½®ï¼š{err:#}")
                            } else {
...
/// natural phrase boundary (`.`, `,`, `:`, `;`, `â€”`, `-`, or whitespace)
/// so words are never split. Appends `â€¦` only when text was actually cut.
