...
//! sizes; the 80-wide golden proves the preview pane sheds. Re-bless with
...
use super::{
    TidelineSettingsRail, TidelineSettingsStage, render_tideline_settings_rail,
    render_tideline_settings_stage, tideline_settings_categories, tideline_settings_rail_hitboxes,
};
...
fn settings_stage_matches_goldens_at_blocker_sizes() {
    for (w, h) in BLOCKER_SIZES {
        assert_matches_golden(&format!("settings_{w}x{h}"), &draw_stage(w, h));
    }
}
...
fn settings_rail_lists_eight_categories_and_meta_rows() {
    ...
    let text = render_golden_text(18, 30, |buf| {
        render_tideline_settings_rail(Rect::new(0, 0, 18, 30), buf, &rail);
    });
    assert_eq!(tideline_settings_categories().len(), 8);
    for label in tideline_settings_categories() {
...
fn live_preview_is_a_real_projection_of_the_real_renderers() {
    ...
    assert!(text.contains("▚△▞"), "real composer fluke cap: {text}");
...
fn settings_stage_sheds_preview_below_hundred_columns() {
    ...
    assert!(narrow.contains("Appearance"), "rail stays: {narrow}");
