...
impl AutomationManager {
    ...
    fn validate_storage_id(id: &str, field: &str) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn rejects_storage_ids_with_path_components() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        let manager = AutomationManager::open(tempdir.path().to_path_buf()).expect("manager");

        assert!(manager.get_automation("../escape").is_err());
        assert!(manager.list_runs("../escape", None).is_err());
        ...
        assert!(manager.save_run(&run).is_err());
        assert!(!tempdir.path().join("escape.json").exists());
    }
