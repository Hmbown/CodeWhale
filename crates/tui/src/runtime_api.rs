...
pub async fn run_http_server(
    ...
    if let Some(bootstrap) = web_bootstrap {
        ...
        if !web_public_origins.is_empty() {
            ...
            println!(
                "This is Tailscale reachability (your tailnet ACL). Account remote control (/rc) is a separate pairing path."
            );
            if options.show_qr || options.tailscale {
                let qr_url = format!("{}/", web_public_origins[0].trim_end_matches('/'));
...
pub fn build_router(state: RuntimeApiState) -> Router {
    ...
    Router::new()
        ...
        .route("/assets/codewhale-mark-dark.png", get(web::web_mark_dark))
        .route("/assets/codewhale-mark-light.png", get(web::web_mark_light))
        .route("/assets/access-qr.svg", get(web::web_access_qr))
        .route(
