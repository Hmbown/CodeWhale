...
# Xiaomi MiMo Token Plan（tp-* 密钥使用 api-key 认证）
XIAOMI_MIMO_TOKEN_PLAN_API_KEY="tp-..." XIAOMI_MIMO_MODE="token-plan-sgp" codewhale --provider xiaomi-mimo --model mimo-v2.5-pro
# 或在配置中设置：[providers.xiaomi_mimo] mode = "token-plan"（中国集群）、"token-plan-sgp" 或 "token-plan-ams"
