...
- `CODEWHALE_RUNTIME_DIR` (override the runtime thread store root; default
  `$CODEWHALE_HOME/tasks/runtime`). The store is single-owner: a second
  process on the same root fails at startup. Use a per-session path to run
  more than one Codewhale on a machine (#5630). Legacy alias:
