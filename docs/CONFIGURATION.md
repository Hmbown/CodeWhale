...
`codewhale auth set --provider arcee --api-key "YOUR_ARCEE_API_KEY"` or
`codewhale auth set --provider deepinfra --api-key "YOUR_DEEPINFRA_API_KEY"`
...
DeepInfra defaults to `https://api.deepinfra.com/v1/openai`, accepts
`DEEPINFRA_BASE_URL`, and uses `deepseek-ai/DeepSeek-V4-Pro` by default with
`deepseek-ai/DeepSeek-V4-Flash` available as the fast lane. Set
`DEEPINFRA_MODEL` or `[providers.deepinfra].model` to override it.
