# Build Codewhale wordmarks from IBM Plex Sans Condensed SemiBold outlines.
# Run: python3 scripts/build-wordmark.py <path-to-IBMPlexSansCondensed-SemiBold.ttf>
import re
import shutil
import sys
...
def main(font_path):
    ...
    hmtx = font["hmtx"]
    tightening = font["head"].unitsPerEm * 0.01
    ...
    for character in TEXT:
        ...
        max_y = max(max_y, top)
        x += hmtx[glyph_name][0] - tightening
    ...
    gradient = (
        f'<defs><linearGradient id="wordmark-gradient" x1="0" y1="0" '
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: python3 scripts/build-wordmark.py <path-to-IBMPlexSansCondensed-SemiBold.ttf>")
    main(sys.argv[1])
